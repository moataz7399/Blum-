...
function startGame() {
  ...
  countdownInterval = setInterval(() => {
    gameTime -= 0.1;
    if (gameTime <= 0) {
        endGame(); // استدعاء الدالة المسؤولة عن ايقاف اللعبة
        gameTime = 0; // تعيين قيمة المتغير بعد ايقاف اللعبة
    }
    document.getElementById('timer').textContent = formatTimerDigits(gameTime.toFixed(2));
}, 100);
...
tasksTaskButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (taskIndex === 0) {
            ...
            let linkToOpen = 'https://youtube.com/@Baoow';
            window.open(linkToOpen, '_blank');
